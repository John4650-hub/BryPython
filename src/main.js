...
class Brython {
  ...
  async init($page, cacheFile, cacheFileUrl) {
    ...
    $page.id = 'acode-plugin-brython';
    this.$page = $page;
    this.$page.settitle('Brython');
  ...
  async run() {
    ...
    this.print('Brython initializing');
    try {
...
if (window.acode) {
  ...
  acode.setPluginInit('acode.plugin.brython', (baseUrl, $page, { cacheFileUrl, cacheFile }) => {
    if (!baseUrl.endsWith('/')) baseUrl += '/';
    brython.baseUrl = baseUrl;
    brython.init($page, cacheFile, cacheFileUrl);
  });
  acode.setPluginUnmount('acode.plugin.brython', () => {
    brython.destroy();
  });
  // future reference
  if (acode.registerShortcut) {
    acode.registerShortcut('Brython Console', brython.terminal.bind(brython), 'Brython');
  }
  if (acode.registerMenu) {
    acode.registerMenu('Brython Console', brython.terminal.bind(brython), 'Brython');
  }
}
